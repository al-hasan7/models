...
class KANLinear(torch.nn.Module):
    def __init__(
        ...
        grid_eps=0.02,
        grid_range=[-1, 1],  # Grid range, default is [-1, 1]
    ):
        ...
        self.base_weight = torch.nn.Parameter(torch.Tensor(out_features, in_features)) # Initialize base weights and piecewise polynomial weights
    ...
    def reset_parameters(self):
        torch.nn.init.kaiming_uniform_(self.base_weight, a=math.sqrt(5) * self.scale_base)  # Use Kaiming uniform initialization for base weights
        with torch.no_grad():
            noise = (# Generate scaling noise
                (
    ...
    def update_grid(self, x: torch.Tensor, margin=0.01): 
        ...
        x_sorted = torch.sort(x, dim=0)[0] # Sort each channel individually to collect data distribution
...
class KAN(torch.nn.Module): # Encapsulates a KAN neural network model for fitting and predicting data.
    ...
    def forward(self, x: torch.Tensor, update_grid=False): # Call each KANLinear layer's forward method to perform the forward pass.
        ...
        Args:
            x (torch.Tensor): Input tensor of shape (batch_size, in_features).
            update_grid (bool): Whether to update the grid. Default is False.
